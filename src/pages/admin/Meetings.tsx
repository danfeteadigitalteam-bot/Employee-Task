  ...
  useEffect(() => {
    ...
    const fetchMeetings = async () => {
      ...
      const [meetingsRes, empCountRes] = await Promise.all([
        supabase.from("meetings").select("*").eq("company", "nte").order("meeting_date", { ascending: false }),
        supabase.from("employees").select("*", { count: "exact", head: true }).eq("is_active", true).neq("role", "admin").contains("companies", ["nte"]),
      ]);
